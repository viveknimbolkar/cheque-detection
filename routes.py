...
def register():
    msg = ''
    if request.method == 'POST' and 'email' in request.form and 'name' in request.form and 'password' in request.form and \
        ...
        if account:
            ...
            cursor.execute(
                'INSERT INTO `user` (`name`, `email`, `password`,`account_no`,`signature`) VALUES (%s, %s, %s, %s, %s)',
                [name, email, password, random_account_number, signaturename])
...
def update_user_details():
    ...
    if request.method == 'POST':
        ...
        if request.files['userimage'].filename != '':
            cursor.execute(query_with_image,
                           (email, name, role, secure_filename(request.files['userimage'].filename), email))
        else:
...
def add_cheque_data():
    if request.method == 'POST':
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'INSERT INTO `history` (`bank_name`, `bearer`, `amount`, `isvalid`, `cheque_no`) VALUES (%s, %s, %s, %s, %s)',
            [request.json['bankName'], request.json['bearer'], request.json['amount'], request.json['isValid'],
             request.json['chequeNo']])
        mysql.connection.commit()
...
def extract():
    if request.method == 'POST':
        if request.files['cheque'].filename != '':
            ...
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute(
                'INSERT INTO `history` ( `bearer`, `amount`, `amount_in_words`,`account_no`,`date`) VALUES (%s, %s, %s, %s,%s)',
                [extracted_data[0], extracted_data[3], extracted_data[2], extracted_data[1], extracted_data[5]])
            mysql.connection.commit()
    ...
    if request.method == 'POST':
        if request.files['cheque'].filename != '':
            ...
            if customer_signature_name:
                customer_signature_path = os.path.join(app.config['UPLOAD_SIGNATURE_FOLDER'],
                                                       customer_signature_name['signature'])
                ...
                if is_signature_valid:
                    cursor.execute(
                        'INSERT INTO `valid_cheque` ( `name`, `account_no`, `amount`,`ifsc`,`date`, `amount_in_words`, `bank_name`) VALUES (%s, %s, %s, %s, %s, %s, %s)',
                        [extracted_account_no[0], extracted_account_no[1], extracted_account_no[3],
                         extracted_account_no[4], extracted_account_no[5], extracted_account_no[2], "SBI"])
                    mysql.connection.commit()
                    flash('verified')
